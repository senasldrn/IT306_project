...
<head>
    ...
    <style>
        ...
        .container { max-width: 1000px; margin: auto; background: white; padding: 25px; border-radius: 12px; box-shadow: 0 8px 25px rgba(0,0,0,0.25); }
        h1 { margin-bottom: 20px; color: #222; }
        form { margin-bottom: 20px; }
        input, select { padding: 8px; margin-right: 10px; margin-bottom: 10px; border: 1px solid #ccc; border-radius: 6px; }
        button, .btn { padding: 8px 14px; background: #ff6b35; color: white; text-decoration: none; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; display: inline-block; margin: 3px; }
        button:hover, .btn:hover { background: #e85a28; }
        table { width: 100%; border-collapse: collapse; margin-top: 15px; }
        th, td { border: 1px solid #ddd; padding: 10px; text-align: left; }
        th { background: #f3f3f3; color: #222; }
        tr:nth-child(even) { background: #fafafa; }
        .top-links { margin-bottom: 15px; }
        .explain { margin-top: 10px; color: #444; background: #fff4ef; padding: 10px; border-left: 4px solid #ff6b35; border-radius: 6px; }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <div class="explain">
        ...
        if ($category != '') echo "Filtering by category: <b>" . htmlspecialchars($category) . "</b>. ";
        if ($zone != '') echo "Showing restaurants in zone: <b>" . htmlspecialchars($zone) . "</b>. ";
    ...
    <table>
        ...
        if (mysqli_num_rows($result) > 0) {
            while($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td>" . $row["zone"] . "</td>";
                echo "<td>
                    <a href='restaurants/restaurant_menu.php?restaurant_id=" . $row["restaurant_id"] . "&zone=" . urlencode($row["zone"]) . "&name=" . urlencode($row["name"]) . "' class='btn'>View Menu</a>
