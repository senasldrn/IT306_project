...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($name == "" || $category == "" || $zone == "") {
        $error = "Rejected because all fields are required.";
    }
    else if ($zone != "A" && $zone != "B") {
        $error = "Rejected because zone must be A or B.";
    }
    else {
        $sql = "INSERT INTO restaurants (name, category, zone) VALUES (?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);

        mysqli_stmt_bind_param($stmt, "sss", $name, $category, $zone);

        if (mysqli_stmt_execute($stmt)) {
            $message = "Restaurant added successfully.";
        } else {
            $error = "Restaurant could not be added.";
        }
